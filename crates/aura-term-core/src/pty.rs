...
//!
//! Consumers use `CommandStart` → open a Block, `CommandEnd` → transition it
//! to Completed/Failed. Raw output between markers flows as `PtyEvent::Output`.
...
impl PtySession {
    pub fn write(&self, bytes: &[u8]) -> Result<()> {
        let mut guard = self
            .writer
            .lock()
            .map_err(|_| Error::Pty("pty writer lock poisoned".into()))?;
        guard.write_all(bytes)?;
        Ok(())
    }

    /// Detach the event stream. The first caller gets the Receiver; subsequent
    /// calls return `None`. Used at init time where the Receiver is moved into
    /// a subscription stream and the rest of the session (writer) is kept
...
pub fn spawn(cfg: PtyConfig<'_>) -> Result<PtySession> {
    ...
    let pty = tty::new(&options, cfg.size, 0)
        .map_err(|e| Error::Pty(format!("pty spawn: {e}")))?;

    let write_file = pty.file().try_clone().map_err(Error::Io)?;
