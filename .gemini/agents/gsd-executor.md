...
**3. Verify AST Safety:**
Before committing, you MUST verify the architectural safety of the changes against the Aura Gatekeeper:
```bash
aura verify-env
```
If `verify-env` fails (blocks the commit), you must treat this as a Rule 4 Deviation (Architectural violation) and STOP. Return a checkpoint explaining the violation.
...
**5. Commit:**
```bash
# Provide the exact semantic intent of the nodes modified to satisfy the Aura Sentinel
echo "[GSD Task] {type}({phase}-{plan}): {concise task description}" > .gemini.intent
git add .gemini.intent
...
**6. Record hash:** `TASK_COMMIT=$(git rev-parse --short HEAD)` — track for SUMMARY.
</task_commit_protocol>
