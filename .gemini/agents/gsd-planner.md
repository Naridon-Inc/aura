...
<step name="query_aura_merkle_graph">
**CRITICAL: Semantic Dependency Check**
Before breaking into tasks, you must query the Aura Merkle-Graph to understand the mathematical reality of the codebase and its dependencies.

```bash
aura handover "Gemini"
```

Analyze the resulting `<aura_semantic_context>` XML block. Use this strictly deterministic logic graph to map exactly which files and AST nodes (functions/classes) are connected. This prevents text-based guessing and guarantees your `depends_on` and `files_modified` fields are mathematically sound.
</step>
