// ─── Aura Merge Engine ──────────────────────────────────────────────────────
// Handles structured merging for all file types:
//   - Code files: scaffold (non-function portions) + AST function merge
//   - JSON/YAML/TOML: key-level deep merge
//   - Text/Markdown: line-level 3-way merge
//   - Everything else: file-level hash compare (safe fallback)
...
pub fn detect_file_type(path: &str) -> FileType {
    ...
    if lower.contains("node_modules/") || lower.contains("target/")
        || lower.contains(".git/") || lower.contains(".aura/")
        || lower.contains("__pycache__/") || lower.contains(".next/")
        || lower.contains("dist/") || lower.contains("build/") {
        return FileType::Ignored;
    }

    // Lockfiles — never auto-merge
    if lower.ends_with("cargo.lock") || lower.ends_with("package-lock.json")
        || lower.ends_with("yarn.lock") || lower.ends_with("pnpm-lock.yaml")
        || lower.ends_with("poetry.lock") || lower.ends_with("gemfile.lock") {
        return FileType::Ignored;
    }
    ...
    if lower.ends_with(".png") || lower.ends_with(".jpg") || lower.ends_with(".jpeg")
        ...
        || lower.ends_with(".woff2") || lower.ends_with(".ttf") || lower.ends_with(".eot")
        || lower.ends_with(".zip") || lower.ends_with(".tar") || lower.ends_with(".gz")
        || lower.ends_with(".exe") || lower.ends_with(".dll") || lower.ends_with(".so")
        || lower.ends_with(".dylib") || lower.ends_with(".pdf") {
        return FileType::Binary;
    }

    // Structured formats
    if lower.ends_with(".json") { return FileType::Json; }
    if lower.ends_with(".yaml") || lower.ends_with(".yml") { return FileType::Yaml; }
    if lower.ends_with(".toml") { return FileType::Toml; }
    if lower.ends_with(".env") || lower.contains(".env.") { return FileType::Env; }

    // Code (AST-parseable)
    if lower.ends_with(".rs") || lower.ends_with(".py") || lower.ends_with(".ts")
        || lower.ends_with(".tsx") || lower.ends_with(".js") || lower.ends_with(".jsx")
        || lower.ends_with(".go") || lower.ends_with(".java") || lower.ends_with(".cs")
        || lower.ends_with(".rb") || lower.ends_with(".cpp") || lower.ends_with(".cc")
        || lower.ends_with(".c") || lower.ends_with(".h") || lower.ends_with(".hpp")
        || lower.ends_with(".php") || lower.ends_with(".swift") || lower.ends_with(".kt") {
...
pub fn merge_json(local: &str, remote: &str, base: Option<&str>) -> MergeResult {
    ...
    let merged = deep_merge_value(&local_val, &remote_val, base_val.as_ref(), "$", &mut conflicts);
    ...
    match (local, remote) {
                    ...
                    (Some(lv), Some(rv)) => {
                        if lv == rv {
                            ...
                            if lv == bv {
                                ...
                                let result = deep_merge_value(lv, rv, Some(bv), &child_path, conflicts);
        ...
        (Value::Array(_), Value::Array(_)) => {
            if local == remote {
                ...
                if local == bv { remote.clone() }
                else if remote == bv { local.clone() }
                else {
...
fn merge_text_3way(base: &[&str], local: &[&str], remote: &[&str]) -> MergeResult {
    ...
    while bi < base.len() || li < local.len() || ri < remote.len() {
        ...
        match (b_line, l_line, r_line) {
            ...
            (Some(b), Some(l), Some(r)) if b != l && b != r => {
                if l == r {
                    ...
                    merged.push(format!("<<<<<<< LOCAL"));
                    merged.push(l.to_string());
                    merged.push(format!("======="));
                    merged.push(r.to_string());
                    merged.push(format!(">>>>>>> REMOTE"));
                }
                bi += 1; li += 1; ri += 1;
            }
        ...
        if bi > max_len + 100 { break; } // safety
    }
...
fn merge_text_2way(local: &[&str], remote: &[&str]) -> MergeResult {
    ...
    // Common prefix
    let prefix_len = local.iter().zip(remote.iter())
        .take_while(|(a, b)| a == b)
        .count();

    // Common suffix (from the end)
    let suffix_len = local.iter().rev().zip(remote.iter().rev())
        .take_while(|(a, b)| a == b)
        .count();
...
pub fn merge_env(local: &str, remote: &str) -> MergeResult {
    ...
    for key in all_keys {
        match (local_map.get(key), remote_map.get(key)) {
            ...
            (Some(lv), None) => { merged.insert(key.clone(), lv.clone()); }
            (None, Some(rv)) => { merged.insert(key.clone(), rv.clone()); }
            (None, None) => {}
        }
    }

    let result: String = merged.iter()
        .map(|(k, v)| format!("{}={}", k, v))
        .collect::<Vec<_>>()
        .join("\n");
...
mod tests {
    ...
    fn test_env_merge() {
        ...
        match merge_env(local, remote) {
            MergeResult::Conflicts { conflict_count, merged, .. } => {
