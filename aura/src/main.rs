...
const CURRENT_VERSION: &str = "0.2.0-alpha";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
 ...
 ░░░       ░░░     ░░░░░░░░░░   ░░░░░   ░░░░░   ░░░       ░░░   
      ...
      S O V E R E I G N   S E M A N T I C   V A U L T"#.cyan().bold());
            ...
            if agent_id != "Aura Continuous Daemon" && !staged_nodes.is_empty() {
                ...
                for node in &staged_nodes {
                    if let Some(ref ident) = node.identifier {
                        ...
                        if intent_lower.contains(&ident.to_lowercase()) {
                            aligned = true;
                        }
                    }
                }
                ...
                if !aligned {
                    ...
                    spinner.println(format!("\n  {} {}", "How to Fix:".bold().green(), "Update your commit message to include the names of the functions or classes listed above."));
        ...
        Commands::VerifyEnv { target, pos_target } => {
            let env_target = target.clone().unwrap_or_else(|| {
        ...
        Commands::Config { sub } => {
            ...
            let options = vec![
                ...
                format!("Embeddings Engine (Currently: {})", if config.use_local_embeddings { "Local (Offline)".green() } else { "Cloud (Gemini API)".blue() }),
                ...
                "Update Gemini API Key".to_string(),
                "Exit".to_string(),
            ];
            ...
            match selection {
                ...
                4 => {
                    let api_key = Password::with_theme(&ColorfulTheme::default())
                        .with_prompt("Enter new Gemini API Key")
