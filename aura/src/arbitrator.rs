...
impl Arbitrator {
    pub fn resolve_conflict(file_path: &str) {
        ...
        println!("  {} Querying Local ML Runner (or Gemini API) to synthesize patch...", "↳".dimmed());
        
        // Use Gemini API as the Arbitrator engine (as a stand-in for a local Ollama model)
        if let Some(api_key) = crate::config::ConfigManager::get_api_key() {
            let url = format!("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro:generateContent?key={}", api_key);
            let client = reqwest::blocking::Client::new();
            
            // Sprint 3: The Context Engine (XML Prompt Synthesis)
            // Instead of passing a raw string, we synthesize a token-optimized XML structure
            // that forces the LLM to understand the architectural boundaries.
            let prompt = format!(
...
{}
            ...
            if let Ok(res) = client.post(&url).json(&body).send() {
                if let Ok(json_res) = res.json::<serde_json::Value>() {
                    if let Some(text) = json_res["candidates"][0]["content"]["parts"][0]["text"].as_str() {
                        let cleaned_code = text.replace("```python", "").replace("```", "").trim().to_string();
            ...
            println!("{} Failed to synthesize patch via API.", "✗".red());
        } else {
            println!("{} GEMINI_API_KEY not found. Arbitrator requires an LLM backend.", "✗".red());
        }
    }
}
