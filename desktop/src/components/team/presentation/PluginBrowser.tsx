...
export function PluginBrowser({ repoRoot }: { repoRoot: string }) {
  ...
  return (
    <div className="h-full overflow-y-auto px-3 py-3 flex flex-col gap-3">
      ...
      {notice && (
        <div className="text-[11px] text-accent-green bg-accent-green/10 border border-accent-green/20 rounded px-2 py-1.5 leading-snug">
          {notice}
        </div>
      )}
      ...
      <section>
        <header className="flex items-center justify-between mb-1.5">
          ...
          <Button
            ...
            className="h-auto px-0 text-[10.5px] text-text-4 no-underline hover:text-text-1 hover:no-underline"
            title="Refresh"
          >
            Refresh
          </Button>
            ...
            {rows.map((row) => (
              ...
              >
                <div className="flex items-baseline justify-between gap-2">
                  <span className="text-text-1 text-[12.5px] font-medium truncate">
                    {row.name}
                  </span>
                ...
                {row.description && (
                  <div className="text-text-4 text-[11px] mt-1 leading-snug line-clamp-2">
                    {row.description}
                  </div>
                )}
                <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                  ...
                  {row.trusted ? (
                    <span className="text-[10px] text-accent-green bg-accent-green/10 rounded px-1.5 py-0.5">
                      ✓ {row.publisher.label}
                    </span>
                  ) : (
                    <span
                      className="text-[10px] text-text-4 bg-bg-1 rounded px-1.5 py-0.5"
                  ...
                  {row.capabilities.slice(0, 3).map((c) => (
                    <span
                      key={c}
                      className="text-[10px] text-text-5 bg-bg-1 rounded px-1.5 py-0.5 font-mono"
                    >
                      {c}
                    </span>
                  ))}
                ...
                {confirmTrust?.publish_id === row.publish_id ? (
                  <div className="mt-2 border border-line-soft rounded px-2 py-1.5 bg-bg-1">
                    ...
                    <div className="text-[10px] text-text-5 font-mono mt-1 break-all">
                    ...
                    <div className="flex gap-1.5 mt-1.5">
                      <button
                        ...
                        className="px-2 py-0.5 text-[10.5px] rounded bg-accent/15 hover:bg-accent/25 text-accent disabled:opacity-50"
                      >
                        Trust &amp; install
                      </button>
                      <Button
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    {row.installed ? (
                      <span className="text-[10.5px] text-text-4">Installed</span>
                    ) : (
                      <button
                        type="button"
                        disabled={busy === row.publish_id}
            ...
            {publishable.map((p) => (
              ...
              >
                <div className="min-w-0">
                  <div className="text-text-2 text-[11.5px] truncate font-mono">
                    {p.bundle ?? p.id}
                  </div>
                  <div className="text-text-5 text-[10px]">v{p.version}</div>
                </div>
                <button
                  type="button"
                  disabled={busy === p.install_dir}
                  onClick={() => void publish(p.install_dir)}
                  className="px-2 py-0.5 text-[10.5px] rounded bg-accent/15 hover:bg-accent/25 text-accent disabled:opacity-50 flex-shrink-0"
                >
