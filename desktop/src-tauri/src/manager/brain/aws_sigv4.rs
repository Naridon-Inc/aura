...
//!
//! Just enough SigV4 to authenticate one Bedrock `InvokeModel` call — we
//! deliberately do NOT pull the AWS SDK (`aws-config` + a smithy runtime is a
//! very large dependency tree for one signed POST). The algorithm is fully
//! deterministic: canonical request → string-to-sign → derived signing key →
//! HMAC signature, exactly as documented at
...
//! `x-amz-security-token` when a session token is present). That covers the
//! Bedrock runtime endpoint and nothing we don't use.
