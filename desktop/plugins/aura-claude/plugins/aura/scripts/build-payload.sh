...
# Negotiate the protocol version with Aura.
# Uses min(plugin_current, aura_declared), falling back to 1 if Aura doesn't
# advertise a version.
...
build_payload() {
    ...
    # Extra args should be jq flag pairs like: --arg key "value" or
    # --argjson key '{"a":1}'
    jq -nc \
